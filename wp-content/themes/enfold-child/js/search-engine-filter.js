...
jQuery(document).ready(function($) {
    ...
    app.utils = (function() {
		...
		function newGetApiCall(url = "/", queryParams = {}, body = {}, name = "search") {
            return new Promise((resolve, reject) => {
                if (app.xhrs[name] !== undefined && app.xhrs[name] !== null) {
                    app.xhrs[name].abort();
                }
    ...
    app.search = (function() {
        ...
        function init() {
            ...
            cache.$documentTabs.on('click', function(e) {
                ...
                if(dataTab !== activeTab) {
                  ...
                  search(1, "/" + (dataTab == 'all' ? 'posts' : dataTab));
                }
            });
            ...
            cache.$searchBtn.on('click', function(e) {
                ...
                search(1, "/" + (activeTab == 'all' ? 'posts' : activeTab));
            });
        ...
        async function search(page = 1, endpoint = "/posts") {
            ...
            if(filters && (filters.business_sectors || filters.domains || filters.functions)) {
              ...
              if(filters.business_sectors && filters.business_sectors.length > 0) {
                for(const id of filters.business_sectors) {
                  newUrl = newUrl + 'business_sector[]=' + id + '&';
                }
              }

              if(filters.domains) {
                for(const id of filters.domains) {
                  newUrl = newUrl + 'domain[]=' + id + '&';
                }
              }

              if(filters.functions) {
                for(const id of filters.functions) {
                  newUrl = newUrl + 'functions[]=' + id + '&';
                }
              }

              console.log('Before', newUrl);
            ...
            try {
                $resultContainer.children('.panel-body').html(preloader);
					      var results = await app.utils.newGetApiCall(endpoint, params, filters, 'search');
        ...
        function renderResults(data) {
          var $resultContainer = getActiveContainer();
          var rows = [];
          cache.$totalResult.text(`${Number(data.length)}`);
          if (data.length > 0) {
              data.forEach((post) => {
                  var row = `
                  <div class="card mb-4">
                    <div class="card-body pt-0 pb-4 px-0">
                      ...
                      <div class="hc-lh-sm ml-3 ml-lg-0">
                        ...
                        <span class="hc-fs-18 hc-fw-400">Emilie Pourier | ${app.utils.formatDate(post.modified)}</span>
                      </div>
          ...
          $resultContainer.find('.panel-footer span.total').html(`Page 1/1`);
